...
5. **Approved deletion requests now scrub PII.** After admin approval: non-nullable identity columns are replaced with sentinels (`email` → `deleted-{id}@deleted.local`, `firstName` → `'Deleted'`, `lastName` → `'User'`, `password` → an unguessable 60-char random hex that is not a valid bcrypt hash); nullable PII columns (`phone`, `displayPicture`, `birthday`, `gender`, `referralCode`, all onboarding/personalization fields) are set to `null`/`[]`; and `PushToken` rows are deleted. Historical FK integrity (payments, attendance, audit logs) is retained but the user can no longer be contacted or logged in as. Admin UIs rendering deleted-member names will see `'Deleted User'`; any code that expects a nullable name must handle `'Deleted'`/`'User'` sentinels too.
